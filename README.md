...
| 🖼 OCR 입력 | 손글씨·사진 일기를 Naver Clova OCR로 글자로 변환해 입력 |
...
| 메시징 | **RabbitMQ (Spring AMQP)** | 일기 생성 요청 큐잉·비동기 처리 |
| 외부 AI | **Gemini · Replicate · Suno · Clova OCR** | 글·그림·음악 생성, OCR |
...
### 멈추지 않는 비동기 생성 파이프라인
일기 작성 요청은 곧장 처리하지 않고 메시지 큐에 적재한 뒤, 백그라운드에서 단계별로 가공합니다.

```
[일기 작성 요청]
      │  (즉시 응답, 상태 = 생성중)
      ▼
  RabbitMQ ── Listener (동시성 3~6)
      │  이벤트 발행
      ▼
  ├─ 1) 글 정제   (Gemini)
...
- 외부 AI는 **WebClient로 논블로킹 호출**, `@Async` 스레드풀로 병렬 처리합니다.
...
### 요청을 끝까지 추적하는 로깅
요청마다 트레이스 ID(`requestId`)를 부여하고, **MDC를 큐·비동기 스레드까지 전파**해 하나의 요청 흐름을 로그로 따라갈 수 있습니다.
