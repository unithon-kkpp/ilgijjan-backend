...
| ✏️ AI 일기 생성 | 손글씨·사진 입력은 OCR(Google Vision)로 글자를 추출하고, 글 정제(Gemini) 이후 그림(Gemini)과 음악(Gemini Lyria)을 병렬로 생성하는 비동기 파이프라인 |
...
| 데이터 | **Spring Data JPA** · **MySQL 8** · **Flyway** | 일기·유저·재화 영속화, 스키마·인덱스 마이그레이션 관리 |
| 캐시 | **Redis** | 공개 피드 등 무거운 조회 결과 캐싱 |
| 비동기 파이프라인 | **Spring Event · @Async · Virtual Thread (JDK 21)** | 일기 생성 요청을 백그라운드로 처리 |
| 외부 AI | **Gemini(텍스트·이미지) · Gemini Lyria(음악) · Google Vision OCR** | 글·그림·음악 생성, OCR |
...
[일기 작성 요청]
  ...
  @Async 가상 스레드에서 백그라운드 처리
      ▼
  1) 글 정제 (Gemini)  ※ 사진 입력은 OCR(Google Vision)로 글자를 먼저 추출
      ▼
  2) 그림 생성 · 음악 생성을 병렬로 실행 (Gemini · Gemini Lyria)
      │
      ▼
  완성  ──┬─ 📱 앱 : FCM 푸시 알림
          └─ 💻 웹 : 상태 폴링 조회 API
...
- 외부 AI는 **WebClient**로 호출하고, 글 정제가 끝나면 **그림 생성과 음악 생성을 병렬로** 처리해 전체 대기 시간을 줄입니다.
...
이미지 생성기는 `ImageGenerator`, 음악 생성기는 `MusicGenerator` 인터페이스로 추상화해, **설정만 바꿔 AI 제공자를 교체**할 수 있도록 했습니다. (현재는 `GeminiImageGenerator` · `LyriaMusicGenerator`가 기본 구현체)
